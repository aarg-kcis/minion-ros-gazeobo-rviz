...
void MPCsolver::load_data(double deltaT, Eigen::Matrix<double, 3, 16> obstacle_force, Eigen::Vector3d ExternalForce, Eigen::Matrix<double, 15, 1> InitialState, Eigen::Matrix<double, 15, 1> TerminalState, Eigen::Matrix<double, 15, 2> StateConstraint, Eigen::Matrix<double, 3, 2> InputConstraint, Eigen::Matrix<double, 18, 1> CostFunction)
{
    ...
    //Input Cost Matrix
    for(int i=0;i<3;i++)
      params.R[i] = CostFunction(15+i);  //Last four entries of cost matrix are control input constraints
