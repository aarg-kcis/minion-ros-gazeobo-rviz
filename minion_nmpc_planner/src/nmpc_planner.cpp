...
void Planner::precompute_AttractiveForce()
{
    ...
    for(int t=0;t<16;t++)
    {
    	...
    	Position3D posDiff (tCurrentSelfPosition(0) - xT,tCurrentSelfPosition(1) - yT,0);
    	double targetDist = posDiff.norm();
    	Position3D targetDirection = posDiff.normalized();


        attractive_force(0,t) = targetDist*targetDirection[0];
        attractive_force(1,t) = targetDist*targetDirection[1];
        attractive_force(2,t) = 0;

        precomputed_force(0,t) = attractive_force(0,t) + obstacle_force(0,t);
        precomputed_force(1,t) = attractive_force(1,t) + obstacle_force(1,t);
        precomputed_force(2,t) = attractive_force(2,t) + obstacle_force(2,t);


        float temp_sum = 0;
        for (int k=0;k<=2;k++)
        	temp_sum += precomputed_force(k,t);
        //ROS_INFO("Sum of Forces = %f", temp_sum);
...
void Planner::selfPoseCallback(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr& msg, int robID) //most important -- self nmpc
{
    ...
    {
        x3 = targetObjectGTPose.pose.position.x + 15 *HUMAN_SPEED*cos(target_yaw)* INTERNAL_SUB_STEP * targetObjectGTVelocity.pose.pose.position.x;
        y3 = -targetObjectGTPose.pose.position.y - 15 *HUMAN_SPEED*sin(target_yaw)* INTERNAL_SUB_STEP * targetObjectGTVelocity.pose.pose.position.y;
    }
    ...
    x4 = x3 + r*cos(theta);//+diminishingOmega*deltaT);
    y4 = y3 + r*sin(theta);//+diminishingOmega*deltaT);
