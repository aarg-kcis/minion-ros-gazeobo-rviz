        ...
        Eigen::Vector3d totalForce = Velocity3D::Zero();
            ...
            for(int j=0; j<numRobots_-1; j++)
            {
                if(heardFromMates[j])
                {
                    ...
                    {
                        if (direction > 0)
                            totalForce +=  0.5*0.1*sumOfGradients/attractionWeight*tangentUnit*potentialForce2 ;
                        else
                            totalForce +=  0.5*0.1*sumOfGradients/attractionWeight*tangentUnitNeg*potentialForce2 ;
                    }
...
void Planner::selfPoseCallback(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr& msg, int robID) //most important -- self nmpc
{
    ...
    outPose.grad = sumOfGradients/attractionWeight;
    outPose.ang = total_ang_force;
